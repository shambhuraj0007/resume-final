...
{
  "personalDetails": {
    "fullName": string,           // Extract from original resume
    "email": string,              // Extract from original resume
    "phone": string,              // Extract from original resume
    "linkedin": string,           // Extract from original resume (or empty string)
    "github": string,             // Extract from original resume (or empty string)
    "website": string,            // Extract from original resume (or empty string)
    "location": string            // Extract from original resume
  },
  "jobTitle": string,             // Professional title optimized for the job
  "objective": string,            // 2-3 sentence professional summary optimized with keywords
  "workExperience": [
    {
      "jobTitle": string,
      "companyName": string,
      "location": string,
      "startDate": string,        // Format: "Jan 2020" or "2020"
      "endDate": string,          // Format: "Present" or "Dec 2022"
      "description": string       // Bullet points separated by newlines, incorporating improvements
    }
  ],
  "education": [
    {
      ...
      "startDate": string,
      "endDate": string,
      "description": string       // Relevant coursework, achievements, GPA if strong
    }
  ],
  "skills": [
    {
      "category": string,         // e.g., "Programming Languages", "Frameworks", "Tools"
      "skills": string            // Comma-separated skills, include missing keywords where relevant
    }
  ],
  "projects": [
    {
      "projectName": string,
      "description": string,      // Optimized with keywords and achievements
  ...
  "customSections": [
    {
      "sectionTitle": string,     // e.g., "Publications", "Awards", "Volunteer Work"
      "content": string
    }
  ]
}
  ...
  const response = await openRouterQueue.add(() =>
    fetch(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        body: JSON.stringify({
          model: "openai/gpt-4o-mini", // Fast, cost-effective for large outputs
          messages: [
            {
              role: "user",
              content: prompt,
          ...
          temperature: 0.7,
          max_tokens: 4096, // Increased for comprehensive resume data
        }),
      }
    )
  );
